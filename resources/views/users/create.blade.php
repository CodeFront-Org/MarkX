...
<div class="container-fluid py-4">
    <div class="card">
        <div class="card-header pb-0 px-3">
            <h6 class="mb-0">Register New {{ ucfirst($role) }}</h6>
        </div>
        <div class="card-body pt-4 p-3">
            <form role="form text-left" method="POST" action="{{ route('users.store', $role) }}" class="multisteps-form__form">
                @csrf
                <div class="mb-3">
                ...
                <div class="text-center">
                    <button type="submit" class="btn bg-gradient-primary btn-lg w-100 mt-4 mb-0">Register {{ ucfirst($role) }}</button>
                </div>
            </form>
        </div>
    </div>
